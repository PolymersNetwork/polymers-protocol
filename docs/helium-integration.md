...
### Software
- **Node.js**: v18+.
- **Solana CLI**: v1.18+ (`npm install -g @solana/cli@1.18.0`).
- **Helium CLI**: v2.0+ (`npm install -g @helium/cli@2.0.1`).
- **Anchor CLI**: v0.30+ (`npm install -g @coral-xyz/anchor-cli@0.30.0`).
- **Phantom Wallet**: v25.9.x+ for wallet management.
- **Supabase CLI**: v1.187.10 (`npm install -g @supabase/supabase-js@2.45.4`).
- **Axios**: v1.7.2 (`npm install axios@1.7.2`).
- **TypeScript**: v5.5.4 (`npm install typescript@5.5.4`).
- **MSW**: v2.0.0 (`npm install msw@2.0.0`).
- **Jest**: v29.7.0 (`npm install jest@29.7.0`).
...
### Accounts
- **Supabase**: For telemetry storage ([supabase.com](https://supabase.com)).
- **Helium**: For hotspot onboarding and rewards ([helium.com](https://helium.com)).
...
- **Solana Devnet**: For testing ([solana.com](https://solana.com)).
...
2. **Export Helium Wallet**:
   ...
   ```
   > **Note**: Update `@helium/sdk` if export fails (`npm install @helium/sdk@2.1.0`).
...
**Rate Limits**: Free tier limited to ~100 requests/minute ([hivemapper.com/pricing](https://hivemapper.com/pricing)). Use exponential backoff for 429 errors.
    ...
    for (const bin of stageBins) {
      ...
      if (result.success) {
        ...
        if (!telemetryOk || !hivemapperOk) {
          console.warn(`Validation failed for ${bin}: telemetry=${telemetryOk}, hivemapper=${hivemapperOk}`);
          await rollbackFirmware(bin);
          await supabase.from('ota_logs').insert({ binId: bin, firmwareFile, status: 'failed', reason: !telemetryOk ? 'invalid_telemetry' : 'stale_hivemapper' });
        } else {
          await supabase.from('ota_logs').insert({ binId: bin, firmwareFile, status: 'success' });
        }
      } else {
        await rollbackFirmware(bin);
        await supabase.from('ota_logs').insert({ binId: bin, firmwareFile, status: 'failed', reason: 'ota_delivery_failed' });
      }
    }
...
- Log rollback reasons (e.g., `invalid_telemetry`, `stale_hivemapper`) to Supabase.
- Trigger rollback if `fill < 0`, `weight < 0`, or Hivemapper device update is stale (>1 hour).
...
5. **Unit Testing**:
   File: `/scripts/__tests__/iot.test.ts`
   ```typescript
   import { sendSmartBinTelemetry } from '../api/iot/smartbins';
   import { rest } from 'msw';
   import { setupServer } from 'msw/node';
   ...
   describe('SmartBin Telemetry', () => {
     beforeAll(() => server.listen());
     afterAll(() => server.close());

     it('sends valid telemetry', async () => {
       const telemetry = { fill: 50, weight: 15, temp: 20, contamination: 0.05 };
       const sig = await sendSmartBinTelemetry('bin_test', telemetry, 'hm_bin_test', 37.7749, -122.4194);
...
npm run test:lstm
npm run test
npm run ota:deploy --bin test_bin --file ./firmware/test.bin
```
...
2. **Hivemapper API Errors**:
   ...
   - **429 Rate Limit Exceeded**: Free tier limited to ~100 requests/minute ([hivemapper.com/pricing](https://hivemapper.com/pricing)). Implement exponential backoff:
     ```typescript
...
For assistance with the Helium DePIN, Polymers Protocol, and Hivemapper integration, leverage the following resources and communities:

- **GitHub Issues**: File issues or contribute to the Polymers Protocol repository at [github.com/PolymersNetwork/polymers-protocol](https://github.com/PolymersNetwork/polymers-protocol) (replace with actual repo URL).
- **Helium Community**:
  - **Helium Discord**: Join [discord.gg/helium](https://discord.gg/helium) for DePIN, LoRaWAN, and HPL discussions.
  - **X Community**: Follow `#HeliumDePIN` or `@Helium` on X (1.2M+ hotspots, September 2025) for updates and tips.
- **Hivemapper Community**:
  - **Hivemapper Discord**: Join [discord.com/invite/FRWMKyy5v2](https://discord.com/invite/FRWMKyy5v2) for API and map data support.
  - **Hivemapper Docs**: Refer to [docs.hivemapper.com](https://docs.hivemapper.com) for API references.
- **Polymers Protocol Community**:
  - **Discord**: Join [discord.gg/polymersprotocol](https://discord.gg/polymersprotocol) (replace with actual invite link).
  - **Telegram**: Connect at [t.me/polymersprotocol](https://t.me/polymersprotocol) (replace with actual link).
  - **X Posts**: Engage with `#PolymersProtocol` or `@PolymersProtocol` (replace with official handle).
- **Solana Community**:
  - **Solana Discord**: Join [discord.gg/solana](https://discord.gg/solana) for Solana Pay and Metaplex support.
  - **Solana Stack Exchange**: Post questions at [solana.stackexchange.com](https://solana.stackexchange.com).

**Contribution Guidelines**:
- Submit pull requests via the Polymers Protocol GitHub repo.
- For bugs or feature requests, create detailed issues with logs, steps to reproduce, and environment details (e.g., Node.js v18.20.4, Solana CLI v1.18.0).
- Join weekly community calls on Discord (check pinned messages for schedules).

**Contact**:
- For Hivemapper API issues, email [support@hivemapper.com](mailto:support@hivemapper.com).
- For Helium queries, contact [support@helium.com](mailto:support@helium.com).

---

### Notes on Diagram Generation
The **SmartBin Flow** Mermaid diagram is optimized for GitHub rendering. If you want a PNG/SVG export:
- **Recommended Format**: Both PNG (for README) and SVG (for scalability).
- **Folder**: Suggest `docs/images/smartbin_workflow.png` and `docs/images/smartbin_workflow.svg`.
- **Tool**: I can simulate `mermaid-cli`:
  ```bash
  mmdc -i smartbin_flow.mmd -o docs/images/smartbin_workflow.png
  mmdc -i smartbin_flow.mmd -o docs/images/smartbin_workflow.svg
  ```
- **Confirmation Needed**: Please specify if you want PNG, SVG, or both, and the target folder. Alternatively, the Mermaid code renders natively on GitHub.

### Final Confirmation
- **Polymers Community Links**: The Discord/Telegram links are placeholders. Please provide official links or confirm if I should search X for `#PolymersProtocol` to verify.
- **Additional Needs**: Do you want a specific Supabase schema for other tables (e.g., `rewards_cache`, `ota_logs`)? More unit tests? Further OTA rollback refinements?
- **Diagram**: Confirm PNG/SVG generation and folder.
